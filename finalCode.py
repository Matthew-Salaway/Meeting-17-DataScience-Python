...
# Here is an emaple of how to create a simple bar graph
...
# Now that we have the data, lets make a bar graph like the one above, with the y-value as total sales
x = topTen["Publisher"]      # labels on x axis
y = topTen['Total Sales']   # Corresponding y-values for labels
plt.bar(x, y)
plt.show()
# Notice how the titles get jumbled, so it may be a good idea to abbreviate the publisher name
